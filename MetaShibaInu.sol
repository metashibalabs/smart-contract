...
contract DividendPayingToken is ERC20, IDividendPayingToken {
    ...
    address public adminAddress = 0xdEcDfB80430C6e31Cab9785AAD63f54C8495b29e;
    ...
    mapping(address => uint256) public metashibaRewardRemains;
    ...
    function _claimDividendOfUserToAnyToken(address payable user, address _wantToken) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            if(!swapped) {
                ...
                metashibaRewardRemains[user] = metashibaRewardRemains[user].add( afterAmount.sub(beforeAmount) );

                bool success = IERC20(_wantToken).transfer(user, metashibaRewardRemains[user]);

                if(!success) {
                    return 0;
                }else{
                    metashibaRewardRemains[user] = 0;
...
contract MetaShibaInu is ERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    address public marketingWallet = 0x5898B2d454f1c097106141cbb806820b0fc80F2A;
    address public treasuryWallet = 0x755f453c3947E5CCe77ea6e9C78733a9cc695b24;
    ...
    constructor() ERC20("Meta Shiba Inu", "MSHIB") {
    ...
    function setBusdDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newBusdDividendTracker.owner() == address(this), "must be owned by MSI");
